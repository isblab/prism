...
def get_coordinates(path, output_base_path, output_path, resolution, molecule): # path to directory containing RMF3 files
    ...
    # sys.exit(0)
    Parallel(n_jobs=-1)(delayed(get_coordinates)(mod_id, str_file) for mod_id,str_file in tqdm(enumerate(sorted_paths)))
    ...
    np.savez(os.path.join(output_base_path, 'all_models_coordinates.npz'), conform, radii)
