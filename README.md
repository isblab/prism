...
For pdb files, ensure that the `return_spread` flag is present. The input here would be the directory containing your pdb files. In the `example\pdbs' directory, run the following:
...
```
python color_precision_pdb.py -i PED00001e001.pdb -pf output/bead_spreads_cl2.txt -o output/pdb_colored_model.pdb
```
